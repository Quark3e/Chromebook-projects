 ...
 * 
 * @param point_A `pos2d<float>` object of point A
 * @param point_B `pos2d<float>` object of point B
...
inline std::vector<float> getCoef_linear(
    ...
    float k, c;
    pos2d<float> delta = pos2d<float>(point_B.x-point_A.x, point_B.y-point_A.y);
    // if(delta.x==0) {
...
/**
 * @brief Get the `pos2d<float>` object for the point/coordinate where two lines A and B intersect.
 * 
 ...
 * @return pos2d<float> of the intersected points. If no intersect is found then `pos2d<float>(-1, -1)` is returned
 */
inline pos2d<float> getIntersect(
    pos2d<float> point_A0,
    pos2d<float> point_A1,
    pos2d<float> point_B0,
    pos2d<float> point_B1,
    std::vector<float>* _coefs_A = nullptr,
    std::vector<float>* _coefs_B = nullptr
) {
    pos2d<float> intersect_pos(0, 0);
 ...
 * @return std::vector<pos2d<float>> of the points that form the bezier curve
 */
inline std::vector<pos2d<float>> quadratic_bezier(
    pos2d<float> point_A,
    pos2d<float> point_B,
    pos2d<float> point_C,
    int segNum,
    ...
    std::vector<pos2d<float>> *_AC = nullptr,
    std::vector<pos2d<float>> *_CB = nullptr
) {
    //numLines = segNum-1
    std::vector<pos2d<float>> curvePoints = std::vector<pos2d<float>>(segNum+2, pos2d<float>());
    ...
    std::vector<pos2d<float>> AC(1, point_A);
    std::vector<pos2d<float>> CB(1, point_C);

    pos2d<float> delta_AC(point_C.x-point_A.x, point_C.y-point_A.y);
    pos2d<float> delta_CB(point_B.x-point_C.x, point_B.y-point_C.y);


    for(int i=1; i<=segNum; i++) {
        AC.push_back(pos2d<float>(point_A.x+i*(delta_AC.x/(segNum)), point_A.y+i*(delta_AC.y/(segNum))));
        CB.push_back(pos2d<float>(point_C.x+i*(delta_CB.x/(segNum)), point_C.y+i*(delta_CB.y/(segNum))));
    }
