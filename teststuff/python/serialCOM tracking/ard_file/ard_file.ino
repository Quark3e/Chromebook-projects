...
float getServo4Offset(float degrees) {
...
void driveServo(int servoNum, int degree, bool useTimer = false) {
    ...
    if(servoNum == 3 || servoNum == 5 || servoNum == 7) {degree = 180 - degree;}
    ...
    else {pwm.writeMicroseconds(servoNum-1, microsec_dur);}
...
void get_Angles(
  ...
  if (isnan(check)) {if (printText) {Serial.print("asin(sqrt(pow(d5x, 2) + pow(d5z, 2)) / d5) is NaN\n");}}
    ...
    (*q5) = asin(sqrt(pow(d5x, 2)+pow(d5z, 2))/(d5+d6));
  }
...
void setup() {
    ...
    s[1] = USMIN + float(USMAX - USMIN) * (float(90)/180);
    s[2] = USMIN + float(USMAX - USMIN) * (float(0 - 180)/180);
    s[3] = USMIN + float(USMAX - USMIN) * (float(90)/180);
    s[4] = USMIN + float(USMAX - USMIN) * (float(135)/180);
...
void loop() {
    ...
    while(true) {
        bool bPos = false; //If bPos == false, a is "reversed"
