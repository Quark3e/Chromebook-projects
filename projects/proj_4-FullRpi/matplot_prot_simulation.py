...
def configure_plot():
    ...
    # on the plane y=0
    ax.view_init(elev=20., azim=-35, roll=0) #type: ignore
...
def FK_solver():
    ...
    P[5] = [P[4][0]+(link[4])*math.sin(read_orient[0])*math.sin(read_orient[1]), P[4][1]+(link[4])*math.cos(read_orient[1])*math.cos(read_orient[0]), P[4][2]+(link[4])*math.sin(read_orient[1])]
    read_PP = [P[4][0]+(link[4]+link[5])*math.sin(read_orient[0])*math.sin(read_orient[1]), P[4][1]+(link[4]+link[5])*math.cos(read_orient[1])*math.cos(read_orient[0]), P[4][2]+(link[4]+link[5])*math.sin(read_orient[1])]
...
def main():
    ...
    while True:
        ...
        orient = [float(opt[0]),float(opt[1]),float(opt[2])]
