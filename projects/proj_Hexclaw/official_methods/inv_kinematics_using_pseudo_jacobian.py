...
class RoboticArm:
    ...
    def position(self,Q,index=-1,p_i=[0,0,0]):
        ...
        # extract the points
        px = running_multiplication[0][0] #type: ignore
        py = running_multiplication[1][0] #type: ignore
        pz = running_multiplication[2][0] #type: ignore
    ...
    def pseudo_inverse(self,theta_start,p_eff_N,goal_position,max_steps=np.inf):
        ...
        delta_p = p_end - p_j #type:ignore  # delta_x, delta_y, delta_z between start position and desired final position of end effector
    ...
    def jacobian(self,Q,p_eff_N=[0,0,0]):
        ...
        for i in range(0, self.N_joints):
          if (first_iter == True):
            ...
            p_eff_minus_this_p = p_eff - self.position(Q,index=i) #type: ignore
